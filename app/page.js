    ...
    return (
      <main className="min-h-screen bg-black text-green-500 font-mono p-4 flex flex-col items-center border-[12px] border-black text-center">
        <div className="w-full max-w-4xl border-2 border-green-500 p-8 flex flex-col items-center min-h-[90vh] relative">
          <header className="mb-12"><h1 className="text-5xl font-black tracking-[0.5em] text-green-400">STAR LEAF</h1></header>
          <button onClick={startScan} className="w-64 h-24 border-2 border-green-400 bg-transparent flex items-center justify-center text-xl tracking-[0.2em] shadow-[0_0_20px_rgba(74,222,128,0.5)] text-green-500">
            {isScanning ? "SEARCHING..." : "SCANNING START"}
          </button>
          <button onClick={() => setView("house")} className="mt-8 text-xs text-yellow-400 underline italic">ヌールマーケットで官給品を調達する >></button>
        </div>
      </main>
    );
  }
  ...
  return (
    <main className={`min-h-screen ${themeStyles.split(' ')[0]} flex flex-col items-center p-8 font-mono`}>
      <header className="text-center mb-6 w-full max-w-2xl border-b pb-4 border-opacity-20">
        <h1 className="text-4xl font-black tracking-widest uppercase italic text-center text-white">NURU MARKET HOUSE</h1>
      </header>
      ...
      <div className="mt-10 grid grid-cols-2 gap-6 w-full max-w-md">
        <button onClick={() => setView("pause")} className="p-6 bg-white/10 border border-white/20 rounded-2xl hover:bg-white/20 transition-all text-center">☕️<div className="text-xs font-bold tracking-widest italic uppercase">Pause</div></button>
        <button onClick={() => setView("star-leaf")} className="p-6 bg-emerald-900/20 border border-emerald-500/30 rounded-2xl hover:bg-emerald-900/40 transition-all text-center text-green-500">🌿<div className="text-xs font-bold tracking-widest italic text-green-500">STAR LEAF</div></button>
      </div>
    </main>
  );
}
